...
Public Class vhicle_del
    ...
    Public Shared Function GetChartData(cellValue As String, frm_year As String, to_year As String) As String
        ...
        TranDt = con.ReturnDtTable("SELECT icm_mst.Loc_Code, MONTH(icm_mst.Inv_Date) AS Mnth, COUNT(icm_mst.Loc_Code) AS Count, icm_mst_amount.Taxable AS Amount FROM icm_mst JOIN( SELECT DMS_ROW_DATA.Bill_No, SUM(DMS_ROW_DATA.Taxable) AS Taxable FROM icm_mst JOIN DMS_ROW_DATA ON icm_mst.dms_inv = DMS_ROW_DATA.Bill_No WHERE icm_mst.Export_Type IN (1,2) AND icm_mst.tran_type = 51 AND icm_mst.Loc_Code='" + loc_co + "' and icm_mst.Inv_Date BETWEEN '" & frm_year & "-04-01' AND '" & to_year & "-03-31' GROUP BY DMS_ROW_DATA.Bill_No) AS icm_mst_amount ON icm_mst.dms_inv = icm_mst_amount.Bill_No WHERE icm_mst.Export_Type IN (1,2) AND icm_mst.tran_type = 51 AND icm_mst.Inv_Date BETWEEN '" & frm_year & "-04-01' AND '" & to_year & "-03-31' GROUP BY icm_mst.Loc_Code, MONTH(icm_mst.Inv_Date), icm_mst_amount.Taxable")
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Private Sub location_Data()
            Try
                ...
                Dim TranDt As DataTable
                TranDt = con.ReturnDtTable("Select Loc_Code,Month(Inv_Date) as Mnth,Count(Loc_Code) as Count from  ICM_MST where ISNull(Veh_Del,0)=1 and ICM_MST.Export_Type in (1,2) and  ICM_MST.tran_type=51 and INV_DATE between  '" & Year_From.Text & "-04-01' AND '" & Year_To.Text & "-03-31' group by loc_code,Month(Inv_Date)")
                ...
                For i As Integer = 0 To BrDt.Rows.Count - 1
                    ...
                    If dtrow.Length > 0 Then
                        ...
                        dt.Rows(dt.Rows.Count - 1)("Apr") = Val(TranDt2.Compute("Max(Count)", "Mnth=" & 4 & "").ToString)
                        dt.Rows(dt.Rows.Count - 1)("May") = Val(TranDt2.Compute("Max(Count)", "Mnth=" & 5 & "").ToString)
                        dt.Rows(dt.Rows.Count - 1)("Jun") = Val(TranDt2.Compute("Max(Count)", "Mnth=" & 6 & "").ToString)
                        dt.Rows(dt.Rows.Count - 1)("Jul") = Val(TranDt2.Compute("Max(Count)", "Mnth=" & 7 & "").ToString)
                        dt.Rows(dt.Rows.Count - 1)("Aug") = Val(TranDt2.Compute("Max(Count)", "Mnth=" & 8 & "").ToString)
                        dt.Rows(dt.Rows.Count - 1)("Sep") = Val(TranDt2.Compute("Max(Count)", "Mnth=" & 9 & "").ToString)
                        dt.Rows(dt.Rows.Count - 1)("Oct") = Val(TranDt2.Compute("Max(Count)", "Mnth=" & 10 & "").ToString)
                        dt.Rows(dt.Rows.Count - 1)("Nov") = Val(TranDt2.Compute("Max(Count)", "Mnth=" & 11 & "").ToString)
                        dt.Rows(dt.Rows.Count - 1)("Dec") = Val(TranDt2.Compute("Max(Count)", "Mnth=" & 12 & "").ToString)

                        dt.Rows(dt.Rows.Count - 1)("Jan") = Val(TranDt2.Compute("Max(Count)", "Mnth=" & 1 & "").ToString)
                        dt.Rows(dt.Rows.Count - 1)("Feb") = Val(TranDt2.Compute("Max(Count)", "Mnth=" & 2 & "").ToString)
                        dt.Rows(dt.Rows.Count - 1)("Mar") = Val(TranDt2.Compute("Max(Count)", "Mnth=" & 3 & "").ToString)
    ...
    Public Sub AmountView()
        Try
            ...
            'Dim TranDt As DataTable
            'TranDt = con.ReturnDtTable("Select Loc_Code,Month(Inv_Date) as Mnth,Count(Loc_Code) as Count,(SELECT Taxable FROM icm_mst JOIN DMS_ROW_DATA ON icm_mst.dms_inv = DMS_ROW_DATA.Bill_No; ) as Amount from  icm_mst where ICM_MST.Export_Type in (1,2) and  ICM_MST.tran_type=51 and INV_DATE between  '" & Year_From.Text & "-04-01' AND '" & Year_To.Text & "-03-31' group by loc_code,Month(Inv_Date)")

            Dim TranDt As DataTable
            TranDt = con.ReturnDtTable("SELECT icm_mst.Loc_Code, MONTH(icm_mst.Inv_Date) AS Mnth, COUNT(icm_mst.Loc_Code) AS Count, icm_mst_amount.Taxable AS Amount FROM icm_mst JOIN( SELECT DMS_ROW_DATA.Bill_No, SUM(DMS_ROW_DATA.Taxable) AS Taxable FROM icm_mst JOIN DMS_ROW_DATA ON icm_mst.dms_inv = DMS_ROW_DATA.Bill_No WHERE icm_mst.Export_Type IN (1,2) AND icm_mst.tran_type = 51 AND icm_mst.Inv_Date BETWEEN '" & Year_From.Text & "-04-01' AND '" & Year_To.Text & "-03-31' GROUP BY DMS_ROW_DATA.Bill_No) AS icm_mst_amount ON icm_mst.dms_inv = icm_mst_amount.Bill_No WHERE icm_mst.Export_Type IN (1,2) AND icm_mst.tran_type = 51 AND icm_mst.Inv_Date BETWEEN '" & Year_From.Text & "-04-01' AND '" & Year_To.Text & "-03-31' GROUP BY icm_mst.Loc_Code, MONTH(icm_mst.Inv_Date), icm_mst_amount.Taxable;")

            'Dim TranDt As DataTable
            'TranDt = con.ReturnDtTable("SELECT month(icm_mst.inv_date)as Mnth,icm_mst.loc_code, SUM(icm_dtl.Cons_Disc + icm_dtl.Exch_Disc + icm_dtl.Corp_Disc + icm_dtl.Cons_Disc1) AS total_discount FROM icm_dtl JOIN icm_mst ON icm_dtl.tran_id = icm_mst.tran_id where  icm_mst.INV_DATE BETWEEN '" & Year_From.Text & "-04-01' AND '" & Year_To.Text & "-03-31' GROUP BY icm_mst.inv_date , icm_mst.Loc_Code")
            ...
            'Chart1.Series("Default").XValueMember = "branch"
            'Chart1.Series("Default").YValueMembers = "Apr"
            'Chart1.DataBind()
            'Amount_Grid.CssClass = "my-gridview"

            'Controls.Add(Amount_Grid)
